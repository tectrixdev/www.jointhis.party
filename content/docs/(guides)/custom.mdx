...
<Tabs items={['Reverse proxy', 'Port forwarding']}>
  <Tab value="Reverse proxy">
    ...
    You should first figure out what ports or connections your service needs.
    You can find this out in their documentation.
    Most applications will use TCP.
    There are exceptions like minecraft bedrock and simplevoicechat.
    This will also be present in their documentation.
    Once again I do recommend just using TCP+UDP.
    ...
    ## Creating our jointhis.party record.

    Now in order to use our custom domain, we'll need to create an A record.
    We only need our subdomain name,
    which will be the part before `.jointhis.party`
    ...
    Mycoolserver will probably be taken,
    so you can make your own creative name for your service/server.
